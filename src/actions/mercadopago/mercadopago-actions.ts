...
import type { DatosEstadoPago, DatosPreferenciaPago } from "@/types/mercadopago";
...
export async function crearPreferenciaPago(turnoId: string): Promise<ActionState<DatosPreferenciaPago>> {
  try {
    ...
    return {
      success: false,
      error: "No se pudo procesar el pago. Intentalo de nuevo.",
    };
  }
}
...
export async function confirmarPagoTurno(
  ...
  try {
    ...
    return {
      success: false,
      error: "No se pudo confirmar el pago. Intentalo de nuevo.",
    };
  }
}
