...
type PropiedadesFilaBarbero = { barbero: BarberoListado; servicios: ServicioOpcion[]; diasLaborales: DiaLaboral[]; soloEdicionPropia?: boolean; menuAbierto: boolean; onAlternarMenu: () => void; onCerrarMenu: () => void };
...
export default function BarberoFila({ barbero, servicios, diasLaborales, soloEdicionPropia = false, menuAbierto, onAlternarMenu, onCerrarMenu }: PropiedadesFilaBarbero) {
  ...
  return <>
    <article className="relative border-b px-4 py-4 transition-colors hover:bg-[var(--admin-item-hover)] last:border-b-0 lg:grid lg:min-h-[104px] lg:grid-cols-[minmax(220px,1.5fr)_minmax(170px,1fr)_minmax(150px,0.8fr)_110px_120px] lg:items-center lg:gap-5 lg:px-6" style={{ borderColor: "var(--admin-border)" }}>
      <div className="flex min-w-0 items-center gap-3">
        ...
        <div className="min-w-0"><p className="truncate font-semibold text-[var(--admin-texto-primario)]">{barbero.nombre}</p><p className="mt-0.5 truncate text-sm text-[var(--admin-texto-muted)]">{barbero.usuario?.email ?? "Sin cuenta asociada"}</p></div>
