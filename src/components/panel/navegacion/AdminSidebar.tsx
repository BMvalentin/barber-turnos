...
function obtenerNavegacionVisible(rol: RolPanel) {
  return GRUPOS_NAVEGACION.map((grupo) => ({
    ...
    .map((grupo) => ({
      ...grupo,
      items: grupo.items.filter((entrada) => "href" in entrada || entrada.items.length > 0),
    }))
  ...
  return (
    <>
      ...
      <aside aria-label="Navegación de administración" className={`fixed inset-y-0 left-0 z-50 flex h-[100dvh] w-[240px] shrink-0 flex-col border-r border-[var(--admin-border)] bg-[var(--admin-surface)] transition-[transform,width] duration-[240ms] ease-[cubic-bezier(0.16,1,0.3,1)] motion-reduce:transition-none md:sticky md:translate-x-0 ${abierto ? "translate-x-0" : "-translate-x-full"} ${colapsado ? "md:w-[68px]" : "md:w-[240px]"}`}>
        <div className={`group/header relative flex h-16 shrink-0 items-center border-b border-[var(--admin-border)] px-4 ${colapsado ? "md:justify-center md:px-0" : "gap-2.5"}`}>
          <Link href="/" onClick={alCerrar} aria-label={`Ir al sitio de ${marca}`} className={`flex min-w-0 flex-1 items-center gap-2.5 ${colapsado ? "md:w-full md:justify-center md:gap-0" : ""}`}>
            <span className={`relative flex h-10 w-10 shrink-0 items-center justify-center ${colapsado ? "md:cursor-pointer" : ""}`}>
            ...
            <span className={`min-w-0 truncate text-sm font-semibold text-[var(--admin-texto-primario)] ${colapsado ? "md:hidden" : ""}`}>{marca}</span>
          </Link>
          {colapsado && (
            <button type="button" onClick={alAlternar} aria-label="Expandir sidebar" aria-expanded="false" className="absolute right-1 top-1 hidden h-6 w-6 items-center justify-center rounded-md text-[var(--admin-texto-secundario)] opacity-0 transition-[opacity,color,background-color,transform] duration-200 ease-out motion-reduce:transition-none hover:scale-105 hover:bg-[var(--admin-border)] hover:text-[var(--admin-texto-primario)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--page-focus-ring)] md:flex md:group-hover/header:opacity-100 md:group-focus-within/header:opacity-100">
